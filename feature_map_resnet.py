...
model.load_state_dict(torch.load("best_checkpoint_resnet.pth", map_location='cpu'))
model.eval()
...
for i in range(len(model_children)):
    if type(model_children[i]) == nn.Conv2d:
        ...
        for j in range(len(model_children[i])):
            ...
            elif str(type(model_children[i][j])) == "<class 'torchvision.models.resnet.ResNet'>":
                conv_layers.append(model_children[i][j])
            else:
...
plt.savefig(str('feature_maps_resnet.jpg'), bbox_inches='tight')
